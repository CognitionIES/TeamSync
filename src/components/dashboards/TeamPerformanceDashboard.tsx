...
import { Download, BarChart3, PieChart as PieChartIcon, Calendar, Users } from 'lucide-react';
...
const TeamPerformanceDashboard = ({ tasks, teamLeads }: TeamPerformanceProps) => {
  ...
  const calculateTeamStats = (): TeamStats[] => {
    ...
    Object.keys(stats).forEach(lead => {
      if (stats[lead].completed > 0) {
        stats[lead].avgCompletionTime = stats[lead].totalTime / stats[lead].completed;
        ...
        stats[lead].performance = Math.min(100, Math.max(0, 
          100 - ((stats[lead].avgCompletionTime - baseScore) / baseScore) * 100
        ));
      }
    });
  ...
  const filterTasksByTimePeriod = (tasksToFilter: Task[]): Task[] => {
    ...
    return tasksToFilter.filter(task => new Date(task.updatedAt) >= cutoffDate);
  };
  ...
  const handleExportCSV = () => {
    // Convert team stats to CSV
    const headers = ["Team Lead", "Assigned", "In Progress", "Completed", "Total Tasks", "Avg Completion Time (Hours)", "Performance Score"];
    const rows = teamStats.map(team => [
      ...
      team.avgCompletionTime.toFixed(2),
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Analyzing team progress and efficiency metrics</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Tabs value={chartType} onValueChange={(value) => setChartType(value as 'bar' | 'pie')} className="w-auto">
          ...
          <CardHeader>
            ...
            <CardDescription>
              ...
              {timePeriod === 'all' ? ' all time' : 
               timePeriod === 'day' ? ' the last 24 hours' :
               timePeriod === 'week' ? ' the past week' :
               timePeriod === 'month' ? ' the past month' : ' the past quarter'}
            </CardDescription>
          </CardHeader>
                ...
                <div key={index} className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge 
                      ...
                      className={`${
                        team.performance > 80 ? 'bg-green-100 text-green-800 hover:bg-green-200' : 
                        team.performance > 50 ? 'bg-amber-100 text-amber-800 hover:bg-amber-200' :
                  ...
                  <div className="grid grid-cols-3 gap-2 text-sm">
                    <div className="p-2 bg-blue-50 rounded text-center">
                      <div className="text-blue-800 font-medium">{team.assigned}</div>
                      <div className="text-gray-500 text-xs">Assigned</div>
                    </div>
                    <div className="p-2 bg-orange-50 rounded text-center">
                      <div className="text-orange-800 font-medium">{team.inProgress}</div>
                      <div className="text-gray-500 text-xs">In Progress</div>
                    </div>
                    <div className="p-2 bg-green-50 rounded text-center">
                      <div className="text-green-800 font-medium">{team.completed}</div>
                  ...
                  <div className="text-sm flex justify-between">
                    ...
                    <span className="font-medium">{formatHours(team.avgCompletionTime)}</span>
                  </div>
                  ...
                  <div className="h-1 bg-gray-100 rounded overflow-hidden">
                    <div 
                      className={`h-1 ${
                        ...
                        team.performance > 50 ? 'bg-amber-500' : 'bg-red-500'
                      }`}
                      style={{ width: `${team.performance}%` }}
                    ></div>
                  </div>
                  
                  {index < teamStats.length - 1 && <hr className="border-gray-100" />}
                </div>
              ))}
            </div>
          </CardContent>
